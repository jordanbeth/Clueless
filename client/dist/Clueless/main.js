...
/* harmony import */ var src_models_CardHelper__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! src/models/CardHelper */ "./src/models/CardHelper.ts");
/* harmony import */ var src_models_InactivePieces__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! src/models/InactivePieces */ "./src/models/InactivePieces.ts");
var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var GameComponent = /** @class */ (function () {
    ...
    GameComponent.prototype.ngOnInit = function () {
            ...
            for (var _i = 0, players_1 = players; _i < players_1.length; _i++) {
                ...
                if (playerPiece != _this.myPlayerPiece) {
                    ...
                    for (var i = 0; i < cards.length; i++) {
                        var card = cards[i];
                        var id = src_models_CardHelper__WEBPACK_IMPORTED_MODULE_5__["CardHelper"].getCardHtmlId(card);
                        _this.myCards[i] = { id: id, value: card };
                    }
                    // console.log(this.myCards);
                }
            }
            var inactivePlayersPieces = msg.inactivePlayersPieces;
            console.log(inactivePlayersPieces);
            src_models_InactivePieces__WEBPACK_IMPORTED_MODULE_6__["InactivePieces"].assignLocations(inactivePlayersPieces);
        ...
        this.socketService.onSuggestionMade().subscribe(function (msg) {
            ...
            _this.lastSuggestedPlayer = suggestedPlayer;
            _this.lastSuggestedRoom = formattedRoom;
            _this.lastSuggestedWeapon = weapon;
            _this.lastSuggestion = piece + " suggested '" + suggestedPlayer + "' in '" + formattedRoom + "' with '" + weapon + "'";
            if (piece != _this.myPlayerPiece) {
                _this.currentStatus = _this.lastSuggestion;
            }
            else {
                _this.currentStatus = "You suggested '" + suggestedPlayer + "' in '" + formattedRoom + "' with '" + weapon + "'";
        ...
        this.socketService.onAccusationMade().subscribe(function (msg) {
            var piece = msg.piece; // Colonel Mustard ... etc
            var accusedPlayer = msg.accusedPlayer;
            var weapon = msg.weapon;
            var room = msg.room;
            var didWin = msg.didWin;
            _this.toastr.info(piece + " accused " + accusedPlayer + " in " + room + " with " + weapon, 'Accusation Made', {
                disableTimeOut: true
            });
            if (didWin) {
                if (piece === _this.myPlayerPiece) {
                ...
                if (piece === _this.myPlayerPiece) {
                    _this.currentStatus = 'You\'re accusation was wrong you may only watch now.';
                }
                _this.toastr.error(piece + "'s accusation was wrong", 'Accusation incorrect');
                ...
                _this.movePieceOnGameboard(piece, 'billiard-room');
                _this.resetBoardColors();
            }
        });
        /**
         * Offer clue
   ...
   */
    ...
    GameComponent.prototype.resetBoardColors = function () {
        for (var i = 0; i < GameComponent_1.boardLocationIds.length; i++) {
            var location_4 = GameComponent_1.boardLocationIds[i];
            var node = document.getElementById(location_4);
    ...
    GameComponent.prototype.movePieceOnGameboard = function (piece, location) {
        ...
        if (player === undefined) {
            var elmToRemove = document.getElementById(src_models_InactivePieces__WEBPACK_IMPORTED_MODULE_6__["InactivePieces"].lookUpPieceId(piece));
            elmToRemove.remove();
            var node_1 = document.createElement('IMG');
            node_1.setAttribute('id', src_models_InactivePieces__WEBPACK_IMPORTED_MODULE_6__["InactivePieces"].lookUpPieceId(piece));
            node_1.setAttribute('src', src_models_InactivePieces__WEBPACK_IMPORTED_MODULE_6__["InactivePieces"].lookUpImageUrl(piece));
            node_1.setAttribute('width', '50px');
            var newElm_1 = document.getElementById(location);
            newElm_1.appendChild(node_1);
            return;
        }
        // The player image
        var node = player.node;
    ...
    GameComponent.prototype.handleOfferClue = function () {
        var elm = document.getElementById('offerClueSelectDropdown');
        var card = elm.value;
    ...
    GameComponent.prototype.removePlayerFromBoard = function (piece) {
        var removedPlayer = this.playersByPiece[piece];
        var removedPlayerId = removedPlayer.playerId;
        console.log("removedPlayerId: " + removedPlayerId);
        var elm = document.getElementById(removedPlayerId);
    ...
    GameComponent.suggestionPlayerIds = ['modalColonelMustard', 'modalMissScarlet', 'modalMrGreen', 'modalMrsPeacock', 'modalMrWhite', 'modalProfessorPlum'];
    GameComponent.suggestionWeaponIds = ['candlestick', 'revolver', 'knife', 'leadPipe', 'rope', 'wrench'];
...
/***/ "./src/models/CardHelper.ts":
/*!**********************************!*\
  !*** ./src/models/CardHelper.ts ***!
  \**********************************/
/*! exports provided: CardHelper */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "CardHelper", function() { return CardHelper; });
var CardHelper = /** @class */ (function () {
    function CardHelper() {
    }
    CardHelper.getCardHtmlId = function (cardName) {
        return CardHelper.CARD_NAME_TO_ID_MAP[cardName];
    };
    CardHelper.getCardServerName = function (htmlId) {
        return Object.keys(CardHelper.CARD_NAME_TO_ID_MAP).find(function (key) { return CardHelper.CARD_NAME_TO_ID_MAP[key] === htmlId; });
    };
    CardHelper.CARD_NAME_TO_ID_MAP = {
        'Colonel Mustard': 'colonelMustard',
        'Miss Scarlet': 'missScarlet',
        'Mr. Green': 'mrGreen',
        'Mrs. Peacock': 'mrsPeacock',
...
/***/ "./src/models/InactivePieces.ts":
/*!**************************************!*\
  !*** ./src/models/InactivePieces.ts ***!
  \**************************************/
/*! exports provided: InactivePieces */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "InactivePieces", function() { return InactivePieces; });
var InactivePieces = /** @class */ (function () {
    function InactivePieces() {
    }
    InactivePieces.assignLocations = function (inactivePieces) {
        for (var _i = 0, inactivePieces_1 = inactivePieces; _i < inactivePieces_1.length; _i++) {
            var piece = inactivePieces_1[_i];
            console.log('now in the assignLocations loop');
            var imageUrl = this.pieceToImageMap[piece];
            console.log(imageUrl);
            var playerId = this.pieceToIdMap[piece];
            var node = document.createElement('IMG');
            node.setAttribute('id', playerId);
            node.setAttribute('src', imageUrl);
            node.setAttribute('width', '50px');
            console.log('we are here now');
            var elm = document.getElementById('billiard-room');
            console.log('can we get here 1');
            elm.appendChild(node);
            console.log('can we get here 2');
        }
    };
    InactivePieces.lookUpPieceId = function (piece) {
        var pieceId = this.pieceToIdMap[piece];
        return pieceId;
    };
    InactivePieces.lookUpImageUrl = function (piece) {
        var imageUrl = this.pieceToImageMap[piece];
        return imageUrl;
    };
    InactivePieces.pieceToImageMap = {
